import cubes from './assets/MSeK5W7fBYbdR7MTmXOXfO9sQE8.avif'
import analytics from './assets/gafvjOKcY1DVZdyRaWiETmyZI.avif'
import growth from './assets/rjGU1GCjPxSGEtw13lCs5jIkLrw.avif'
import chat from './assets/N5itSKOFGXUajSDKYfTPt3jso.avif'
import handshakeOverlay from './assets/AeD4G2F6xtDTfPZxF3cBhE7iaE.avif'
import caseFresh from './assets/GyGy9tdLhGReRc1wiQfUj7GHQY.avif'
import caseKondam from './assets/nMPpbIPs8beEk0SdtFkFUU24.avif'
import blog1 from './assets/iFkvcRsxff1AtSWFrurThWF6pfU.avif'
import blog2 from './assets/JC7iAp2EcPtq2537iOxZJexXA4.avif'
import blog3 from './assets/zmIOWbheBAJYTG7wJ41LPmSk.avif'
import logoipsum from './assets/PdCL5aErZ3l6ER5HSGkaRP5JWOI.svg'

const p = (file: string) => `/images/${file}`

export const img = {
  cubes,
  analytics,
  growth,
  chat,
  handshakeOverlay,
  caseFresh,
  caseKondam,
  blog1,
  blog2,
  blog3,
  logo: p('9JYQyVXx8KcGAaFl7ov5ZZGsXs.svg'),
  heroHands: p('finalhero.png'),
  badge: p('hfLbzDwAMFhAQJjaQotR5S5lGc.png'),
  heroBg: p('CXf1VtliCLhgE72rnxykPb20LI8.png'),
  handshake: p('YUzSyGKGhbl5eDTwnRH51Jltc.png'),
  nicolas: p('N5lfRjlzKaEo4Fk3FnOsVIAu20E.png'),
  caseLokos: p('l38NofRiCurjAYNe3fKBaKHpM9k.png'),
  caseRight: p('FQI1rKHWG7UFuIKMD8lLRahD4c.png'),
  logoA: p('rQxLtPn3xA6bRnsG3GsmK1U5NQ.svg'),
  logoB: p('oulRiOylWi1CpWm18i22kqRHqo.svg'),
  logoC: p('C67cxfy5fhDEysnESHz2r7MMG74.svg'),
  logoD: p('eOgGAoQ8cUerr93T63t57197lKs.svg'),
  caseLogo: logoipsum,
  quoteLogo: logoipsum,
  avatar1: p('jQaQy3MeB1acfC0mGXVnuEA.png'),
  avatar2: p('iA1QUmlnNIh548FqXiePXkiTOE.png'),
  avatar3: p('XuwB34O04rgEumYHa8z2gYrPjeQ.png'),
  avatar4: p('Blu51nsth3wyeoA8UWAFJXPARCc.png'),
  avatar5: p('Y3azNG5Wkm1pzKNts3LdJsa2ts.png'),
  chevron: p('vEcNCNvwFVKx5Hb4oHbqI81Idig.svg'),
  iconNlp: p('58JixNY1LrY4tG546W3MKZjvt8.svg'),
  iconStrategy: p('4Sm4xVttryAnfudwSQqg27udqmE.svg'),
  iconCustom: p('ysoj5Gre6vR15Vo00c57Nd9OHFw.svg'),
  iconMl: p('wuyHWOQ4RMr9xbjNH4YuR37j7gE.svg'),
  iconRpa: p('1PoLn7kic1JNPwECBgP8V7XYI.svg'),
  iconWorkflow: p('LzPBtaHdymAROYB08IBc75GUdqY.svg'),
  iconDeploy: p('iQSfPi1dWWpgZ9D8UHbgEo7DyM.svg'),
  iconSupport: p('mmJJAqPsFJuLFLto0o9YFJp4I.svg'),
}
